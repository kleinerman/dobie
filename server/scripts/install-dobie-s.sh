...
[Service]
Type=oneshot
ExecStart=/bin/bash -c 'UNTIL_DATE_TIME=\$\$(date --date "$STORED_MONTHS month ago" +%%Y-%%m-%%d\ %%H:%%M); docker run --name db-purger --rm --network dobie_dobie-net -v $(realpath ../back_end):/dobie-venv/dobie-server -v /var/log/dobie-s:/var/log/dobie-s -v /var/lib/dobie-pers-imgs:/var/lib/dobie-pers-imgs dobie/${IMG_PRFX}backend python -u /dobie-venv/dobie-server/purgeevents.py -d "\$\${UNTIL_DATE_TIME}"'
EOL
...
# A container is created using the backend image which has the PATH variable with /dobie-venv at the
# beginning (See the Dockerfile). For this reason the Python interpreter and all the pip packages installed
# in /dobie-venv will be used.
# During the creation of this container the python source code was mapped inside the directory of the virtual
# environment. Maybe the last thing is not necessary
