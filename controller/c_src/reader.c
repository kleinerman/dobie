...
int init_reader(reader_t *reader_p,
    ...
    sd_journal_print(LOG_NOTICE, "Reader %d of Door: %d using GPIO: %d for D0 and GPIO: %d for D1\n",
                     side, door_id, d0_gpio_num, d1_gpio_num);
    reader_p->chip = chip;
...
void *run_reader (void *arg_p){
    int ret, gpio_num, mask, card_number;
    ...
    while (!exit_flag) {
        ...
        if (!exit_flag) {
            card_number = (card_number & W26_MASK_CLEAN_PARITY_BITS) >> 1;
